...
export default function PhoneCoversPage() {
  ...
  return (
    <div className="bg-background text-foreground">
      {/* Hero Section */}
      <AnimatedSection className="relative bg-gradient-to-r from-teal-500 via-cyan-500 to-sky-500 text-white overflow-hidden">
        ...
        <Container className="relative z-10 py-12 md:py-24 lg:py-32 min-h-[50vh] md:min-h-[70vh] flex items-center">
          <div className="text-center md:text-left max-w-2xl">
            <Smartphone className="h-10 w-10 md:h-16 md:w-16 mb-4 text-white/80 mx-auto md:mx-0 animate-float" />
            <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-bold font-headline mb-3">
              {heroContent.subTitle}
            </h1>
            <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold mb-4 md:mb-6 text-sky-100">
              {heroContent.title}
            </h2>
            ...
            <Link href="#phone-covers-grid" passHref scroll={false}>
              <Button 
                variant="outline" 
                size="lg" 
                className="border-white text-white hover:bg-white hover:text-teal-600 transition-colors duration-300 px-6 md:px-8 py-2 md:py-3 text-sm md:text-base font-semibold group"
              >
      ...
      <AnimatedSection>
        <section id="phone-covers-grid" className="scroll-mt-16">
          <Container className="py-8 md:py-16">
            ...
            {phoneCoverProducts.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 md:gap-6">
                {phoneCoverProducts.map((product, index) => (
                  <div 
                    key={product.id}
              ...
              <p className="text-center text-muted-foreground text-lg py-10">No phone covers available at the moment.</p>
            )}
