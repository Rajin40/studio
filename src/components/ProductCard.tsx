...
import { mockCategories } from '@/lib/data'; // Import mockCategories
...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const productCategoryDetails = mockCategories.find(c => c.name === product.category);
  const isShoeProduct = productCategoryDetails?.id === 'footwear' || productCategoryDetails?.parentCategoryId === 'footwear' || 
                        ['Sneakers', 'Formal Shoes', 'Running Shoes'].includes(product.category); // Simple check for now

  const productLink = isShoeProduct ? `/shoes/${product.id}` : `/products/${product.id}`;
    ...
    <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
      ...
      <CardContent className="p-4 flex-grow">
        <Link href={productLink} className="block">
          <CardTitle className="text-lg font-headline mb-1 leading-tight hover:text-accent transition-colors line-clamp-2">{product.name}</CardTitle>
        </Link>
